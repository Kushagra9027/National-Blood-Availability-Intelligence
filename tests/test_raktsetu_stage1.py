"""
RaktSetu — Stage 1 (Verification & Priority Engine) — Full API Test Script
============================================================================

WHAT THIS DOES
- Hits every endpoint (root, /submit-request, /queue/peek, /queue/pop,
  /queue, /requests/{id}/audit) with a battery of valid and invalid cases.
- Verifies status codes AND response content, not just "did it not crash".
- Verifies priority-queue ordering (Critical > Urgent > Routine > Scheduled,
  FIFO within the same level).
- Prints a clean PASS/FAIL report and a final summary count.
